...
function updateViewBounds(id2, bounds) {
  const view = views.get(id2);
  if (!view || !win) return;
...
ipcMain$1.handle("browser-view:create", (_, id2, url, bounds) => {
  if (!win) return;
  if (views.has(id2)) {
    const view2 = views.get(id2);
...
ipcMain$1.handle("browser-view:update-bounds", (_, id2, bounds) => {
  updateViewBounds(id2, bounds);
});
ipcMain$1.handle("browser-view:load", (_, id2, url) => {
  const view = views.get(id2);
  if (view) {
...
ipcMain$1.handle("browser-view:destroy", (_, id2) => {
  var _a, _b;
  const view = views.get(id2);
  if (view && win) {
    try {
...
ipcMain$1.handle("browser-view:control", (_, id2, action) => {
  const view = views.get(id2);
  if (!view) return;
  switch (action) {
    case "back":
...
ipcMain$1.handle("browser-view:zoom", (_, id2, factor) => {
  const view = views.get(id2);
  if (view) {
    view.webContents.setZoomFactor(factor);
  }
});
ipcMain$1.handle("browser-view:hide", (_, id2) => {
  const view = views.get(id2);
  if (view && win) {
